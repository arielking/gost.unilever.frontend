...
<v-layout align-start>
    <v-container fluid grid-list-sm>
      ...
      <!--VENTANA IMAGEN-->
       ...
       <v-dialog
        v-model="dialogiamgen"
        height="550px"
        width="600px"
        
        
      >
     <template v-for="(item) in this.datasrc">
         ...
         :key="item.idanomalia"
          ...
          <v-img
            ...
            width="600px"
             ...
             :src="(item.foto_anomalia)"
          >
            <v-container fill-height fluid>
              <v-layout fill-height>
                <v-flex xs12 align-end flexbox>
                  <span class="headline">{{ item.idanomalia }}</span>
                </v-flex>
              </v-layout>
            </v-container>
          </v-img>
          ...
          <v-card-actions>
            <v-btn @click="OculatarImagen()" flat color="orange">CERRAR</v-btn>
            <v-btn flat color="orange">CAMBIAR IMAGEN</v-btn>
          </v-card-actions>
        </v-card>
         </template>
      </v-dialog>
      ...
      >
        ...
        {{props.item.descripcion}}</div>
          ...
          <v-card-actions>
            ...
            <v-btn flat icon color="#64DD17"   @click="editItemConfirmar(props.item)" >
                <v-icon>done_outline</v-icon>
              </v-btn>

             <v-btn flat icon color="#0277BD"   @click="editItem(props.item)" >
                <v-icon>launch</v-icon>
              </v-btn>
           
              <v-btn flat icon color="#E65100"  @click="Verfoto(props.item)" >
                <v-icon>present_to_all</v-icon>
              </v-btn>

               <v-btn flat icon color="#455A64"  @click="Verfoto(props.item)" >
                <v-icon>edit</v-icon>
              </v-btn>
          </v-card-actions>
        </v-card>
    ...
    export default {
      props: {
        ...
        OculatarImagen(){
            ...
            this.dialogiamgen=false;
            this.datasrc=[];
        },

             Verfoto(item) {
                
                this.dialogiamgen=true;
                this.listarfoto(item.idregistroanomalia);
                ...
                listarfoto(id){
                    let me=this;
                    this.idregistroanomalia=id;
                    axios.get('api/RegistrosAnomalias/VerFotoCreacion/'+me.idregistroanomalia).then(function(response){
                        //console.log(response);
                        me.datasrc=response.data;
            ...
            listarsinfoto(tarjeta){
                    ...
                    var bandera1;
                    axios.get('api/RegistrosAnomalias/ListarTarjetaSf/'+me.idtarjeta).then(function(response){
                        //console.log(response);
